...
Note from the [[nix-interview-prep|N-iX interview prep]] — micro-frontend architecture.
...
- **Versioning:** If the shared state contract changes, all MFEs need to update —
  see [[shared-component-library-versioning|shared library versioning]].
- **Lazy Loading:** Avoid importing the entire store into every MFE — use
  selective slices. Related: [[react-code-splitting|code-splitting]].
- **Testing:** Both patterns are testable; event bus requires spying on events,
  shared state can be mocked easily.

---

Back to the index: [[nix-interview-prep|N-iX — Interview Prep]].
